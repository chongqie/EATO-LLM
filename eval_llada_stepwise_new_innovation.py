...
from generate_parallel_structure import two_stage_generate_for_eval
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        if not os.path.exists(adapter_path):
            adapter_path = "path/to/adapter"  # Fallback path, adjust as necessary
    ...
    def generate_until(self, requests: list[Instance]):
        ...
        pad_token_id = self.tokenizer.convert_tokens_to_ids("<|reserved_token_0|>")
        out = []
        ...
        for req in tqdm(requests, desc="Generating"):
            ...
            # input: [prompt] + [question] + [prefix]
            prompt_ids = self.tokenizer(prompt_text, return_tensors="pt").input_ids[0]
            question_ids = self.tokenizer(f"Question:{question}\n", return_tensors="pt").input_ids[0]
            ...
            gen_len = 512
            mask_tensor = torch.full((gen_len,), mask_token_id, dtype=torch.long)
            input_ids = torch.cat([input_ids, mask_tensor], dim=0).unsqueeze(0).to(self.device)
            ...
            prompt_len = len(prompt_ids) + len(question_ids) + len(prefix_ids)
